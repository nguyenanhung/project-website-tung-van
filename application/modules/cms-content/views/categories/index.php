            ...
            <div class="page-toolbar">
                <div class="btn-group pull-right">
                    ...
                    <ul class="dropdown-menu pull-right" role="menu">
                        <li><a href="<?php echo site_url($this->modules_link.'/create/createCategory'); ?>"><i class="icon-plus"></i> Thêm mới</a></li>
                        <li class="divider"></li>
...
<!--                                        </div>-->
                        ...
                        <table class="table table-striped table-bordered table-hover" id="table_desc">
                            ...
                            <tbody>
                            <?php
                            foreach($data['item_lists'] as $item) {
                                ?>
                                <!-- BEGIN MODAL CONFIRM DELETE ITEM -->
                                <div class="modal fade" id="<?php echo 'td_confirm_delete_' . encodeId_Url_byHungDEV($item->id); ?>" tabindex="-1" role="basic" aria-hidden="true" style="display: none;">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                Bạn có chắc chắn muốn xóa: <?php echo $item->name; ?> ?
                                            </div>
                                            <div class="modal-footer">
                                                <?php echo form_open($this->modules_link.'/delete'); ?>
                                                <input type="hidden" name="delete_id" value="<?php echo encodeId_Url_byHungDEV($item->id); ?>">
                                                <button type="submit" name="Delete_the_Item" value="<?php echo config_item('confirm_key_delete'); ?>" id="Delete_the_Item_btn_<?php echo encodeId_Url_byHungDEV($item->id); ?>" class="btn sbold red"><i class="fa fa-trash-o"></i> Xóa</button>
                                                <?php echo form_close(); ?>
                                ...
                                <tr class="odd gradeX">
                                    ...
                                    <td><?php echo ($this->sub_categories->get_info_categories($item->parent)) ? $this->sub_categories->get_info_categories($item->parent)->name : "Không có"; ?></td>
                                    <td>
                                        <?php echo ($this->menu->get_info_menu($item->menu_id)) ? $this->menu->get_info_menu($item->menu_id)->title : "Không có"; ?></td>
                                    <td><?php echo $item->language ?></td>
                                    <td class="text-center <?php echo ($item->status == 1) ? " bold uppercase" : "" ?>">
                                        <span>
                                            <?php echo ($item->status == 1)? "Kích hoạt" : "Không kích hoạt";  ?>
                                        </span>
                                        <br>
                                        <?php $btn = ($item->status == 1)? 'danger': 'info'; ?>
                                        <button type="submit" data="<?php echo $item->id ?>" data-style="<?php echo $item->status ?>" class="btn btn-<?php echo $btn;?> btn-circle sbold change_categories_status"><i class="fa fa-exchange"></i></button>
                                        <?php echo form_close(); ?>
                                    </td>
                                    <td><?php echo $item->title; ?></td>
                                    <td><?php echo $item->created_at; ?></td>
                                    <td><a href="<?php echo site_url($this->modules_link.'/edit/'.encodeId_Url_byHungDEV($item->id)); ?>" class="btn btn-circle btn-sm green"><i class="fa fa-edit"></i> Sửa</a></td>
                                    <td>
                                        <a class="btn btn-sm btn-circle sbold dark" data-toggle="modal" href="<?php echo '#td_confirm_delete_' . encodeId_Url_byHungDEV($item->id); ?>"><i class="fa fa-trash-o"></i> Xóa</a>
                                    </td>
                                </tr>
                            <?php } ?>
                            </tbody>
